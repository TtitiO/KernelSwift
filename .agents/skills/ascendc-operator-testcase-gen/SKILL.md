...
在 `ascendc-operator-design` 创建 `design.md` 之后、`ascendc-operator-precision-eval`/`ascendc-operator-performance-eval` 调用之前调用。UT 实现由当前工程的测试约定负责；本 skill 只生成可复用的用例与标杆文档。
