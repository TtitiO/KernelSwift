...
description: >-
  在 ascend-kernel 的算子测试目录下维护仅含 JSONL 的 profiler 性能用例，使用 torch_npu.profiler（固定 warmup=5、active=5）采集，汇总 op_statistic.csv 的 Total Time(us)，输出含 DType 列的统一 Markdown 对比报告（自定义算子 vs 标杆）。不生成额外的 perf_cases 或 profiler 结果 JSON。参考实现见本技能目录的 layer_norm_profiler_reference 示例。
---
