...
description: Kernel直调工程模板，用于创建 Ascend C Kernel 直调工程项目。提供经过验证的 Vector 样例工程（add_custom）、Blaze Matmul 工程模板（纯 Matmul / 融合 / MX 量化 / GroupMatmul）和 Kirin Vector 模板。触发：当用户需要创建 Kernel 直调工程、学习 Ascend C 编程、快速原型验证、或提及 Kernel 直调、kernel launch、Blaze Matmul 或 matmul 模板时使用本 skill。
---
