...
def _get_op():
    if not _LOADED[0]:
        ...
        _LOADED[0] = True
    return torch.ops.indexer_ops.apply_causal_mask
...
# 【优化 1】：纯实数版 RoPE，消除 NPU 对 Complex 类型支持差导致的巨大开销
...
class ModelNew(torch.nn.Module):
    def __init__(self, args, freqs_cis, kv_cache, compress_ratio=4):
        super().__init__()
        self.dim = args.dim
    ...
    def forward(self, x: torch.Tensor, qr: torch.Tensor, start_pos: int, offset: int):
        ...
        pad_t = (actual_t + 15) // 16 * 16
        
        kv_slice = self.kv_cache[:bsz, :actual_t]
        pad_len = pad_t - actual_t
        kv_pad = F.pad(kv_slice, (0, 0, 0, pad_len))
        
        # 保持原汁原味的 PyTorch einsum 计算，保证精度完全不丢！
        index_score = torch.einsum("bshd,btd->bsht", q, kv_pad)
        reduced_scores = (index_score.relu_() * weights.unsqueeze(-1)).sum(dim=2)

        # 【优化 2】：调用 Ascend C 算子，实现零分配的极速 In-place 掩码
        if start_pos == 0:
            _get_op()(reduced_scores.contiguous(), ratio, actual_t)
            
        valid_scores = reduced_scores[:, :, :actual_t]
        topk_idxs = valid_scores.topk(min(self.index_topk, actual_t), dim=-1)[1]
